...
export class Home extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            itemQuant: [],
            // activeItems contains strings, either "" or "is-active", if an item is chosen or not"
            activeItemClasses: [],
            items: [],
            barIsHidden: true
        }
        ...
        .then((response) => {
            ...
            response.data.map((data, index) => {
                ...
                this.setState({
                    ...
                    activeItemClasses: this.state.activeItemClasses.concat("")
                });
    ...
    resetPurchases = () => {
        this.setState({
            ...
            activeItemClasses: this.state.activeItemClasses.fill(""),
            totalSum: 0
        });
    }
    ...
    onMinus = (event, elem) => {
        ...
        // deactivating item by setting to std border color (if needed)
        if(newItemQuantites[elem.indexId] === 0) {
            let tempItemList = this.state.activeItemClasses
            tempItemList[elem.indexId] = ""

            this.setState({
                activeItemClasses: tempItemList
            });
        }
    ...
    onPlus = (event, elem) => {
        ...
        // activating item by changing border color (if needed)
        if(this.state.itemQuant[elem.indexId] === 0) {

            let tempItemList = this.state.activeItemClasses
            tempItemList[elem.indexId] = "is-active"
    ...
    render() {
        ...
        return(
            ...
            <div>
                {console.log(this.state.activeItemClasses)}
                ...
                <div className="items-list">

                    {
                        itemsFiltered.map((elem) => {
                            return <div key={elem.indexId} className={"items-flex " + this.state.activeItemClasses[elem.indexId]}>
